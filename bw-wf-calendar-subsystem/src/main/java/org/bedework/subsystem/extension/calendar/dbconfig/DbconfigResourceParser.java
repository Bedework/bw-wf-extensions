 ...
 */
public class DbconfigResourceParser {
  public static void read(final List<ModelNode> list,
    ...
    final PathAddress addr = PathAddress.pathAddress(
            ...
            PathElement.pathElement(DbconfigResourceDefinition.TAG_NAME,
    ...
    while (reader.hasNext() && (reader.nextTag() != END_ELEMENT)) {
      ...
      final SimpleAttributeDefinition def =
              DbconfigResourceDefinition.lookup(tagName);
      if (def == null) {
  ...
  public static void write(final XMLExtendedStreamWriter writer,
    ...
    if (!context.getModelNode().
                get(DbconfigResourceDefinition.TAG_NAME).
                isDefined()) {
      return;
    }

    writer.writeStartElement(DbconfigResourceDefinition.TAG_NAME);

    final ModelNode elements =
            context.getModelNode().
                   get(DbconfigResourceDefinition.TAG_NAME,
                       BwExtensionDefinitions.defaultValue);
    for (final AttributeDefinition def: DbconfigResourceDefinition.ALL_ATTRIBUTES) {
